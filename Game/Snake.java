...
public class Snake extends Creature
{
    ...
    public void render( Graphics g){
        if (count <40){
            g.drawImage(Assets.snake1, (int) x, (int) y, width, height, null);
        }else{
            if(count == 100){
                count = 0;
            }
            g.drawImage(Assets.snake2, (int) x, (int) y, width, height, null);
        }
        count++;
        //health
        Color black = new Color(255,255,255);
        g.setColor(black);
        g.drawRect(1000-201,50,201,26);
        g.setColor(Color.green);
        g.fillRect(999-199,51,20*getHealth()-1,25);
        g.setColor(black);
        g.drawString("Snake HP",999-200,51+40+1);
